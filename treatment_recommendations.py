...
#this is the treatment recommendations rag system. 
# It reads the treatments data from an excel file, embeds the information using a sentence transformer model, and stores it in a ChromaDB database.
...
#this function retrieves treatment recommendations based on the disease name, severity level, and location provided by the user.
# It first creates a query by combining the disease name and severity level, then embeds the query using the same embedding model used for the database.
# It performs a search in the ChromaDB database for matching entries, prioritizing those that match the severity level.
# If a match is found, it retrieves the disease name, severity, symptoms, and treatments
# associated with the best match. It then constructs a prompt for the Ollama language model to generate a user-friendly response based on the retrieved information.
# Finally, it logs the query and results to a CSV file and returns the output as a JSON object. 
# If no match is found, it returns a message indicating that no match was found and lists the available diseases.
...
#get_treatment_recommendation("blister blight", "low", "Hatton")