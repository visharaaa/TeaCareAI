--  ENUMS
CREATE TYPE user_type_enum AS ENUM ('farmer', 'agronomist', 'admin');
...
CREATE TABLE field (
    ...
    field_latitude      DECIMAL(10, 7)  NOT NULL,
    field_longitude     DECIMAL(10, 7)  NOT NULL,
...
--  5. user_scan_history(user <---> field <---> scan)
...
CREATE TABLE user_scan_history (
    ...
    CONSTRAINT user_scan_history_user_fk        FOREIGN KEY (user_id) REFERENCES "user"(user_id) ON DELETE CASCADE,
...
-- Fast lookup of all fields owned by a user
...
-- Fast lookup of all scans by a user
